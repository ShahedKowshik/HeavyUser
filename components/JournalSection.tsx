    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 self-stretch md:self-auto">
          <div className="flex bg-[#f3f2f1] p-1 rounded border border-[#edebe9]">
            {(['All', 'Log', 'Gratitude'] as JournalFilter[]).map((f) => (
              <button 
                key={f} 
                onClick={() => setFilter(f)} 
                className={`flex-1 sm:flex-none px-3 py-1.5 text-[10px] font-black uppercase rounded transition-all ${filter === f ? 'bg-white text-[#0078d4] shadow-sm' : 'text-[#605e5c] hover:bg-[#edebe9]'}`}
              >
                {f}
              </button>
            ))}
          ...
          <div className="relative flex-1 sm:flex-none">
            ...
            <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 pr-3 py-2 text-xs w-full sm:w-32 md:focus:w-48 font-bold bg-white border border-[#edebe9] rounded transition-all" />
          </div>
      ...
      <div className="space-y-8">
        {groupedJournals.length === 0 ? (
          <div className="text-center py-24 bg-white rounded border border-[#edebe9] border-dashed">
          ...
          groupedJournals.map(([date, entries]) => (
            <div key={date} className="space-y-3">
              ...
              <div className="grid gap-4 grid-cols-1 lg:grid-cols-2">
                {entries.map((entry) => (
                  <div key={entry.id} className="group bg-white rounded border border-[#edebe9] p-5 hover:shadow-lg hover:border-[#d1d1d1] transition-all flex flex-col justify-between relative">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
          <div className="bg-white w-[95%] md:w-full max-w-2xl rounded shadow-2xl animate-in zoom-in duration-200 overflow-hidden flex flex-col max-h-[95vh]">
            <div className="flex items-center justify-between px-8 py-6 border-b border-[#f3f2f1]">
              ...
              <button onClick={closeModal} className="p-2 text-[#a19f9d] hover:bg-[#f3f2f1] rounded transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleSave} className="p-8 flex-1 overflow-y-auto space-y-8 no-scrollbar">
              <div className="space-y-2">
                ...
                <div className="flex p-1 bg-[#f3f2f1] rounded border border-[#edebe9]">
                  {(['Log', 'Gratitude'] as EntryType[]).map((t) => (
                    <button 
                      key={t} 
                      type="button" 
                      onClick={() => setEntryType(t)} 
                      className={`flex-1 py-2 text-xs font-bold rounded transition-all ${entryType === t ? 'bg-white text-[#0078d4] shadow-sm' : 'text-[#605e5c] hover:bg-[#edebe9]'}`}
                    >
                      {t}
                    </button>
                  ))}
              ...
              <div className="space-y-2">
                ...
                <input autoFocus required type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Give this moment a name..." className="w-full text-base font-bold bg-[#faf9f8] border-none rounded p-4 focus:ring-2 focus:ring-[#0078d4]/10" />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black text-[#a19f9d] uppercase tracking-widest">Narrative</label>
                <textarea required rows={6} value={content} onChange={(e) => setContent(e.target.value)} placeholder="Describe the feeling..." className="w-full text-sm font-medium bg-[#faf9f8] border-none rounded p-4 resize-none focus:ring-2 focus:ring-[#0078d4]/10" />
              </div>
            </form>
            <div className="px-8 py-6 border-t border-[#f3f2f1] bg-[#faf9f8] flex justify-end gap-4">
              <button type="button" onClick={closeModal} className="px-6 py-2.5 text-sm font-bold text-[#605e5c] hover:bg-[#edebe9] rounded transition-all">Cancel</button>
